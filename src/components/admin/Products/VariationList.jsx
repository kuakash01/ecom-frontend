...
const ProductVariationsView = ({
  ...
  /* ---------------- STABLE VARIANT INDEX MAP ---------------- */

  const variationIndexMap = useMemo(() => {
    const map = {};
  ...
  }, [variations, groupedByColor, reset]);
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="bg-black text-white px-4 py-2 rounded flex gap-1"
          >
          ...
          return (
            ...
            >
              ...
              {group.variations.map(v => {
                ...
                const idx = variationIndexMap[v._id];
                const watched = watchedVariants?.[idx];

                const changed =
                  Number(watched?.price) !== Number(v.price) ||
                  Number(watched?.mrp) !== Number(v.mrp) ||
                  Number(watched?.quantity) !== Number(v.quantity);
